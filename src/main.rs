...
thread::spawn(move || {
    ...
    let (mut width, mut height) = terminal_size()?;
     ...
     // Create snowdrops
     let mut snowdrops: Vec<Snowdrop> = (0..width).map(|x| Snowdrop::new(x, rng.gen_range(1..=height), snows.choose(&mut rng).unwrap().to_string())).collect();
...
if *resized.lock().unwrap() {
    ...
    *resized.lock().unwrap() = false;
     ...
     // Update existing snowdrops
     for drop in &mut snowdrops {
         drop.x = rng.gen_range(1..=width);
         drop.y = rng.gen_range(1..=height);
         drop.style = snows.choose(&mut rng).unwrap().to_string();
     }
}
